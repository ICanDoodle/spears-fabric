...
public class WoodenSpearItem extends SpearItem {
    ...
    public WoodenSpearItem(ToolMaterial mat, int attackDamage, float attackSpeed, Item.Settings settings) {
        super(mat, attackDamage, attackSpeed, settings);
        ImmutableMultimap.Builder<EntityAttribute, EntityAttributeModifier> builder = ImmutableMultimap.builder();
        builder.put(EntityAttributes.GENERIC_ATTACK_DAMAGE, new EntityAttributeModifier(ATTACK_DAMAGE_MODIFIER_ID, "Tool modifier", 2.0D, EntityAttributeModifier.Operation.ADDITION));
            ...
            if (i >= 10) {
                ...
                if (j <= 0 || playerEntity.isTouchingWaterOrRain()) {
                    if (!world.isClient) {
                        ...
                        if (j == 0) {
                            WoodenSpearEntity spearEntity = new WoodenSpearEntity(modentities.WOODEN_SPEAR,world, playerEntity, stack);
