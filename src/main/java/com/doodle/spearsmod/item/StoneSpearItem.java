...
public class StoneSpearItem extends SpearItem {
    ...
    public StoneSpearItem(ToolMaterial mat, int attackDamage, float attackSpeed, Settings settings) {
            ...
            if (i >= 10) {
                ...
                if (j <= 0 || playerEntity.isTouchingWaterOrRain()) {
                    if (!world.isClient) {
                        ...
                        if (j == 0) {
                            StoneSpearEntity spearEntity = new StoneSpearEntity(modentities.STONE_SPEAR,world, playerEntity, stack);
