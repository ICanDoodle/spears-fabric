...
public class IronSpearItem extends SpearItem {
    ...
    public IronSpearItem(ToolMaterial mat, int attackDamage, float attackSpeed, Settings settings) {
            ...
            if (i >= 10) {
                ...
                if (j <= 0 || playerEntity.isTouchingWaterOrRain()) {
                    if (!world.isClient) {
                        ...
                        if (j == 0) {
                            IronSpearEntity spearEntity = new IronSpearEntity(modentities.IRON_SPEAR,world, playerEntity, stack);
